...
def get_data():
    with open(getpath('smb/cnet_data.json'), 'r') as f:
        data = json.load(f)
...
def train_cnet(cfgs):
    ...
    torch.save(model, getpath(f'{cfgs.save_path}/Cnet.pth'))
    torch.save(model.state_dict(), getpath(f'{cfgs.save_path}/Cnet_state_dict.pth'))
    pass

def get_cnet(path='models/Cnet.pth', device='cpu'):
    safe_path = getpath(path)
