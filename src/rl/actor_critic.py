...
#
#
# class SquashedLSTMActor(nn.Module):
#     LOG_STD_MAX = 2
#     LOG_STD_MIN = -32
#
#     def __init__(self, obs_dim, act_dim, hidden_size, stacks=2):
#         super().__init__()
#         self.obs_dim = obs_dim
#         self.lstm = nn.LSTM(obs_dim, hidden_size, num_layers=stacks, batch_first=True)
#         self.head = nn.Linear(obs_dim, act_dim)
#
#     def forward(self, obs, deterministic=False, with_logprob=True):
#         net_out = self.net(obs)
#         mu = self.mu_layer(net_out)
...
class MLPQFunction(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation=nn.SiLU, m=1):
        super().__init__()
